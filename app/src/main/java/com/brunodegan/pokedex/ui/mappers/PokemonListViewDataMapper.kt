...
class PokemonListViewDataMapper :
    BaseMapper<PokemonsGraphQLApiModel?, List<PokemonListViewData>> {
    override fun map(input: PokemonsGraphQLApiModel?): List<PokemonListViewData> {

        val pokemonListViewDataList = mutableListOf<PokemonListViewData>()
        input?.data?.pokemon?.pokemonNodes?.forEach { apiData ->
    ...
    private fun List<PokemonImages>?.getImageUrl(): String {
        return this?.firstOrNull()?.sprites ?: ""
    }
}